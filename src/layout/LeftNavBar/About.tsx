...
import { APP_NAME, APP_NAME_EN, APP_NAME_ZH, APP_VERSION, SDK_VERSION } from "@/config";
...
export const AboutContent = ({ closeOverlay }: { closeOverlay?: () => void }) => {
  ...
  const displayAppName = t("placeholder.appName") || APP_NAME_ZH;
  const displayVersion = `${displayAppName} ${APP_VERSION}`;
  ...
  const handleCopy = () => {
    copyToClipboard(`${displayVersion} (${APP_NAME_EN})/${SDK_VERSION}`);
      ...
      <div className="bg-[var(--chat-bubble)]">
        ...
        <div className="flex flex-col items-center justify-center">
          ...
          >
            ...
            {displayAppName !== APP_NAME && <div>{APP_NAME}</div>}
            <div>{SDK_VERSION}</div>
          </div>
        </div>
