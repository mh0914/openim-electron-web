...
i18n.on("languageChanged", () => {
  ...
  profileMenuList[1].title = t("placeholder.logOut");
});
...
const LeftNavBar = memo(() => {
  ...
  const selfOperatorUserSet = usePlatformOperators(selfInfo.userID ? [selfInfo.userID] : []);
  const isSelfPlatformOperator = Boolean(
    selfInfo.userID && selfOperatorUserSet.has(selfInfo.userID),
  );
  ...
  return (
    ...
    >
      <div className="mt-6 flex flex-col items-center">
        ...
        {isSelfPlatformOperator && (
          <div
            className="mb-3 flex h-[52px] w-12 cursor-pointer flex-col items-center justify-center rounded-md hover:bg-[#e9e9eb]"
            onClick={() => setBroadcastOpen(true)}
          >
            <NotificationOutlined className="text-[20px] text-[#6b7280]" rev={undefined} />
            <div className="mt-1 text-xs text-gray-500">{t("broadcast.entry")}</div>
          </div>
        )}
      </div>
