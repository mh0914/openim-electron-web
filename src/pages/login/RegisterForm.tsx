...
import { IS_SKIP_SMS_VERIFY, LOCAL_VERIFY_CODE } from "@/config";
...
const RegisterForm = ({ loginMethod, setFormType }: RegisterFormProps) => {
  ...
  const skipVerifyStep = IS_SKIP_SMS_VERIFY;
  ...
  const onFinish = (fields: FormFields) => {
    ...
    const verifyCode = skipVerifyStep ? LOCAL_VERIFY_CODE : code.join("");
  ...
  return (
    <div className="flex flex-col justify-between">
      ...
      <div className={clsx("mt-4 text-2xl font-medium")}>
        ...
        {registerForm === 1 && !skipVerifyStep && <span>{t(verifyTitile)}</span>}
        {registerForm === 2 && <span>{t("placeholder.setInfo")}</span>}
      </div>
      <div
        className="mt-4 tracking-wider text-gray-400"
        hidden={registerForm !== 1 || skipVerifyStep}
      >
