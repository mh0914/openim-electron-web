...
import { parseGroupProfileExtra } from "@/pages/chat/queryChat/GroupSetting/groupProfileExtra";
...
const CommonLeft: FC<ICommonLeftProps> = ({
  ...
  const checkInGroup = async (list: CheckListItem[]) => {
    ...
    const blacklistedUserIDs = new Set(blacklistedMembers.map((item) => item.userID));
    ...
    try {
      ...
      tmpList.map((item) => {
        item.disabled = data.includes(item.userID!) || blacklistedUserIDs.has(item.userID!);
      });
    } catch (error) {
      console.error(error);
    }
